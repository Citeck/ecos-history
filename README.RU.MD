...
Добро пожаловать в репозиторий Citeck `ecos-history`! Репозиторий содержит функциональность отслеживания истории и аудита для платформы Citeck  — системы для автоматизации бизнес-процессов и управления корпоративными данными.
...
Если ваша IDE поддерживает прямой запуск Spring Boot-приложений, вы можете просто запустить класс 'ru.citeck.ecos.history.HistoryApp' без дополнительных настроек.
...
Чтобы убедиться, что всё работает, остановите оригинальный контейнер ecos-history и запустите ecos-history:custom вместо него.
...
Если вам нужна помощь или у вас есть вопросы по Citeck `ecos-gateway`, то создайте issue в этом репозитории или обратитесь в нашу [службу поддержки](mailto:support@citeck.ru).

## Лицензия

Citeck `ecos-gateway` распространяется под лицензией [GNU Lesser General Public License](LICENSE).